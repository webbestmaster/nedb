                ...
                [
                    function (cb) {
                        ...
                        cursor.exec(function (err, docs) {
                            ...
                            assert.equal(docs.find(doc => doc.age === 5).age, 5);
                            assert.equal(docs.find(doc => doc.age === 57).age, 57);
                            assert.equal(docs.find(doc => doc.age === 52).age, 52);
                            assert.equal(docs.find(doc => doc.age === 23).age, 23);
                            assert.equal(docs.find(doc => doc.age === 89).age, 89);
                            cb();
                        });
                    },
                    function (cb) {
        ...
        test('Using one sort', function (done) {
            ...
            cursor.exec(function (err, docs) {
                ...
                for (i = 0; i < docs.length - 1; i += 1) {
                    assert.equal(docs[i].age < docs[i + 1].age, true);
                }
        ...
        test('Sorting strings with custom string comparison function', function (done) {
            ...
            db.find({})
                ...
                .exec(function (err, docs) {
                    assert.equal(_.pluck(docs, 'name')[0], 'zulu');
                    assert.equal(_.pluck(docs, 'name')[1], 'alpha');
                    assert.equal(_.pluck(docs, 'name')[2], 'charlie');

                    delete db.compareStrings;
                    db.find({})
                        .sort({name: 1})
                        .exec(function (err, docs) {
                            assert.equal(_.pluck(docs, 'name')[0], 'alpha');
                            assert.equal(_.pluck(docs, 'name')[1], 'charlie');
                            assert.equal(_.pluck(docs, 'name')[2], 'zulu');
